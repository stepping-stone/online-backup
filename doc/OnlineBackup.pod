=for comment
This document is in Pod format.  To read this, use a Pod formatter, like "perldoc perlpod".

=head1 Online Backup for UNIX
...
<a href="OnlineBackup.tgz">Get Online Backup for UNIX</a>

=end html

=head2 1.3 New Installation
...
Change to the directory where you wish OnlineBackup be installed (by default, the archive will be extracted to a subdirectory OnlineBackup):

C<< cd /root >>

=item 2.

Unpack the gzipp'ed tar archive OnlineBackup.tgz:

C<< gunzip -c ~/Download/OnlineBackup.tgz | tar xvf - >>
...
C<< cp OnlineBackup.conf.default OnlineBackup.conf >>

C<< cp OnlineBackupIncludeFiles.conf.default OnlineBackupIncludeFiles.conf >>

C<< cp OnlineBackupExcludeFiles.conf.default OnlineBackupExcludeFiles.conf >>

=item 5.

Edit B<OnlineBackup.conf>, at least you should set/change the following important parameters:

C<< REMOTEUSER=<username on remote host> >>

C<< PRIVKEYFILE=<path of your private key file> >>; Please make sure that this key isn't protected with a passphrase if you want to run the backup script automatically, e.g. as a cron job!
...
C<< REMOTEHOST=<fully qualified host name> >>

C<< CURRENTPREFIX=<subdirectory under user's home directory> >>; Parent path must exist on remote host!

Further options you may set are described under L<4.1 Main configuration file>

=item 6.

Edit B<OnlineBackupIncludeFiles.conf> and add all files and directories that you want to back up on a seperate line. Please see the examples within the file. Note that all files and directories under a given directory will also be backed up! 

B<Important:> Make sure to include also the path you have assigned to parameter PERMSCRIPT in main configuration file because this file contains important information used for restoring permissions and ownerships!

More detailed description about wildcard patterns, etc. you will find under L<4.2 Inclusion configuration file>

=item 7.

Edit B<OnlineBackupExcludeFiles.conf> and add all files or directories that you want to exclude on a seperate line. Please see the examples within the file. Note that all files and directories under a given directory will also be excluded!

More detailed description about wildcard patterns, etc. you will find under L<4.3 Exclusion configuration file>
...
C<< cat OnlineBackup.log >>

If a message "Backup finished with errors" appers, then something went wrong, see messages above to see the cause and also consult L</6 Troubleshooting>.

Also check the files on the remote host or better, do a restore to a test directory with:

C<< /root/OnlineBackup/OnlineRestore.pl -s current -c /root/OnlineBackup/OnlineBackup.conf -d /var/tmp/test_restore/ >>

If you have a huge amount of data, you may restore only a small subset of your files, e.g. only the path /home/test2/test:

C<< /root/OnlineBackup/OnlineRestore.pl -s current -c /root/OnlineBackup/OnlineBackup.conf -d /var/tmp/test_restore/ -f /home/test2/test >>
...
C<< 10 3    * * *      root    /root/OnlineBackup/OnlineBackup.pl -c /root/OnlineBackup/OnlineBackup.conf >>

B<Important>: Please make sure that you account for the Online Backup logfile (per default OnlineBackup.log) in your logfile rotation mechanism else it will grow forever!
...
If you followed this guide, your configuration files won't be overwritten. Instead, you will find *.default files, B<OnlineBackup.conf.default> contains examples for using new parameters.
...
C<< /root/OnlineBackup/OnlineRestore.pl -s current -c /home/mike/OnlineBackup/OnlineBackup.conf -d /var/tmp/test_restore/ >>

If you have a huge amount of data, you may restore only a small subset of your files, e.g. only the path /home/test2/test:

C<< /root/OnlineBackup/OnlineRestore.pl -s current -c /home/mike/OnlineBackup/OnlineBackup.conf -d /var/tmp/test_restore/ -f /home/test2/test >>

=back

=head1 2 Backup

=head2 2.1 OnlineBackup.pl
...
C<< OnlineBackup.pl -c <configuration file> >>
...
=head2 2.2 Principle of function
...
C<< rsync --exclude-from=<EXCLUDEFILE>  --delete -rlHtvze "ssh -i <PRIVKEYFILE>" --files-from=- <LOCALDIR> <REMOTEUSER>@<REMOTEHOST>:/<CURRENTPREFIX>/<REMOTEDIR> >>
...
C<< OnlineRestore.pl -s <snapshot> [-c configfile] [-f from] [-d destination] where <snapshot> can be current | daily.0-6 | weekly.0-3 | monthly.0-11 >>

=over 1

=item -s	Snapshot:	(mandatory)

B<current:>		the last backed up data

B<daily.0-6:>	the daily snapshot from last day to 7 days in the past

B<weekly.0-3:>	the weekly snapshot from the last back to 4 weeks

B<monthly.0-11:>	from the last to 12 months in the past

=back

=over 1

=item -c	Configuration file:	(optional) 
...
=item -f 	Source (From) path for partial restore:	(optional)
...
=item -d	Destination (To) path for redirected restore:	(optional)
...
C<OnlineRestore.pl -s daily.5 -c /root/backup.conf -f /home/test/ -d /var/tmp/restore>
...
Make sure that networking is started by issuing ifconfig.
If not, load the appropriate kernel module for your ethernet card with

C<< modprobe <modulename> >>

then start the network with

C</etc/init.d/net.eth0 >

=item 3.

Get the archive OnlineBackup.tgz, e.g.

C<wget http://www.cyberbyte.ch/Linux/OnlineBackup/OnlineBackup.tgz>

=item 4.

Unpack the archive in your home directory:

C<tar xvzf OnlineBackup.tgz>
...
C<chmod 400 .ssh/backup_id_dsa>

=item 6.

Get /etc/fstab or /etc/mtab from Online Backup Server, e.g.

C<< scp -o IdentityFile=.ssh/backup_id_dsa <USER>@online-backup.stepping-stone.ch:/incoming/<REMOTEDIR>/etc/fstab . >>
...
C<< scp -o IdentityFile=.ssh/backup_id_dsa <USER>@online-backup.stepping-stone.ch:/incoming/<REMOTEDIR>/root/OnlineBackup/.Partitions.txt . >>

C<< sfdisk /dev/hda < .Partitions.txt >> (or how ever you named the file containing partition information as parameter PARTITIONFILE within OnlineBackup.conf)
...
I<This step is only necessary if you have excluded /dev, possibly because you use udev.>
...
C<cat fstab> (find line with /boot in the second column, hda is the disk device, hda3 would be the partition device)

C<ls -l /dev/hda /dev/hda3> (for the link target) and

C<ls -l -L /dev/hda /dev/hda3> (for the dereferenced file information) of the target you want to restore to
...
- Change to the OnlineBackup directory with C<cd OnlineBackup>.

- Copy/Move B<OnlineBackup.conf.default> to B<OnlineBackup.conf> and modify it as needed, important parameters:

B<REMOTEUSER> and B<PRIVKEYFILE>

B<RSYNCBIN> to C<$HOME/OnlineBackup/rsync> (test if needed to define by typing C<which rsync>) 

B<SSHBIN> to the path of ssh if necessary (test if needed to define by typing C<which ssh>)

B<PERMSCRIPT> to the path it was written on the original installation
...
C<./OnlineRestore.sh current|daily.x|weeky.x|monthly.x OnlineBackup.conf /mnt/gentoo> (new root path)

If you get error messages showing files are searched at a wrong home directory, e.g. "/" instead of "/root", try this:

C<HOME=/root ./OnlineRestore.sh current|daily.x|weeky.x|monthly.x OnlineBackup.conf /mnt/gentoo> (new root path)
...
=head2 4.1 Main configuration file
...
Shell variables ($<variable>) may be used.
Attention: Not all shell variables are available.

=head2 4.2 Inclusion configuration file
...
=head2 4.3 Exclusion configuration file
...
A full description of rsync's exclusion mechanism can be found in the manual page of rsync. Here's a brief summary of the possibilities you have:
...
If a filename stated in the EXCLUDE-file should contain a pattern character (*, **, ?, []), you must escape it with a backslash in front of it (e.g. test\*file).
Also a backslash "/" that is meant literally must be escaped by a second one.
...
=head1 6 Troubleshooting

=head2 6.1 Cannot authenticate on backup host

No backup is being created, the logfile says something like:

C<Permission denied (publickey).>

C<rsync: connection unexpectedly closed (0 bytes received so far) [sender]>

C<rsync error: unexplained error (code 255) at io.c(453) [sender=2.6.9]>

- Check if private key file denoted with PRIVKEYFILE in configuration file exists and is readable by the user and B<NOT accessible> by B<group or others>
...
=head2 6.2 Some files could not be written on backup host

Following error message appears in OnlineBackup logfile:

C<mkstemp "/incoming/mikenoti/test/.changedfile.6MjLwh" failed: Permission denied>

-> check if option REMOTEPERMS in configuration file is set to 1 and set it if not (REMOTEPERMS=1)

=head2 6.3 Deleted files cannot be removed on backup host

Following error message appears in OnlineBackup logfile:

C<delete_one: unlink "/incoming/mikenoti/test/minpermdir2/datei8" failed: Permission denied>

-> check if you are using B<rsync> version newer or equal than B<2.6.7>, those versions can adapt permissions to always allow modifying the containing directory. If you have a current enough version, B<run OnlineBackup.pl again>, because the containing directory wasn't accessible last time. B<Upgrade> if you are using an older version if possible or change the permissions of the local containing directory (temporarily) to at least user writable.

=head2 6.4 Permissions aren't restored correctly
...
=head2 6.5 Error message beginning with "default_perms_for_dir:" appears

Following error message appears in log file and OnlineBackup exits with an non-zero return status:

C<default_perms_for_dir: sys_acl_get_file(test/testdir/subdira, SMB_ACL_TYPE_DEFAULT): No such file or directory, falling back on umask>

-> check if option REMOTEPERMS in configuration file is set to 1 and set it if not (REMOTEPERMS=1)

=head2 6.6 Error message "Creation of partition file failed!" appears
...
-> Check the log file for further information about the cause of this message, and check the partition file that should be created. If it's available but empty, try to use the option SCANDISKS in main configuration file, e.g:
SCANDISKS=/dev/hda /dev/hdb
